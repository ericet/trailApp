      ...
      <div class="col-span-1 lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <h2 class="text-sm font-medium text-gray-600 dark:text-gray-400">Today's Upvotes</h2>
              <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ upvotes.length }}</p>
            </div>
          </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <h2 class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Posts</h2>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <h2 class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Earnings</h2>
              <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ totalEarnings }} STEEM</p>
            </div>
          </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <h2 class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Followers</h2>
              <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ totalFollowers }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Monthly Upvotes</h2>
          ...
          <div v-else class="space-y-4">
            <div class="mb-4">
              <p class="text-sm text-gray-600 dark:text-gray-400">
                Total upvotes this month: {{ upvotes.length }}
              </p>
            </div>
            <div v-for="vote in sortedUpvotes" :key="vote.id" 
              ...
              <div class="flex justify-between items-start">
                <div>
                  <a :href="'https://steemit.com/@' + vote.voter + '/' + vote.permlink" target="_blank" 
            ...
            <div v-for="post in posts" :key="post.url" 
              ...
              <div class="mt-2 flex justify-between items-center text-sm">
                ...
                <div class="flex items-center text-gray-500 dark:text-gray-400">
                  <span class="mr-2">{{ post.votes }} votes</span>
                  <span>{{ post.payout }} STEEM</span>
                </div>
              </div>
            </div>
          </div>
    ...
    async fetchUserHistory() {
      try {
        ...
        this.posts = posts.map(post => ({
          ...
          payout: parseFloat(post.pending_payout_value.split(' ')[0]).toFixed(2)
        }));
        
        this.totalPosts = posts.length;
        this.totalEarnings = posts.reduce((sum, post) => 
          sum + parseFloat(post.pending_payout_value.split(' ')[0]), 0
        ).toFixed(2);
